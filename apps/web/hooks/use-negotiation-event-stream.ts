...
export function useNegotiationEventStream(
  ...
  /**
   * A view already rendered on the server, which is how a snapshot deployment
   * arrives with data. When present there is nothing to fetch: the recording is
   * already here, and the cold-read effect below is skipped.
   */
  initialView: NegotiationView | null = null,
): NegotiationStreamState {
  const [view, setView] = useState<NegotiationView | null>(initialView);
  const [negotiationId, setNegotiationId] = useState<string | null>(
    initialNegotiationId ?? initialView?.negotiationId ?? null,
  );
  ...
  useEffect(() => {
    if (initialNegotiationId === null || initialView !== null) return;
    let cancelled = false;

...
export async function GET(request: Request): Promise<Response> {
  ...
  // Nothing to stream from a recording: the whole run is already in the page.
  // Refusing here beats holding a connection open that will never emit.
  if (!canRunLive()) {
    return new Response(
      "This instance is replaying a recorded negotiation. There is no live stream.",
      { status: 409 },
    );
  }

  const { readNegotiationView, runError } = await import(
    "@/lib/negotiation-run-registry"
  );

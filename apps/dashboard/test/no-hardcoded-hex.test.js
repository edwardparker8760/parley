...
test("the stylesheet actually imports the theme, rather than redefining it", () => {
  const css = readFileSync(join(APP_ROOT, "app", "globals.css"), "utf8");
  assert.ok(
    css.includes('@import "@parley/theme/tokens.css"'),
    "globals.css must read its tokens from the shared theme",
  );
});
...
test("the scan looked at the files that matter", () => {
  ...
  for (const required of [
    ...
    join("components", "live-transcript-ladder.tsx"),
    join("components", "simulated-settlement-badge.tsx"),
  ]) {
